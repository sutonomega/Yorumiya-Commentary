...
- 実 mp4 review の出力契約を確認する test / manual command がある。
- event suppression の test がある。
- MVP event detection acceptance cases が trace で確認できる。
- Ollama comment adapter boundary と template fallback の test がある。
- audio analyzer / VAD の test がある。
- memory による重複抑制の test がある。
- `Comment -> SpeechItem -> voice adapter -> SpeechAudio` boundary の test がある。
