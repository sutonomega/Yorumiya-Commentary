...
M4 では scene / audio / transcript の event を同じ `CommentaryEvent` として扱う。

- scene event: 画面変化に反応する。
- audio event: impact / excited / active な音に反応する。
- transcript event: 人の発話を event selection と suppression の両方から観測する。

発話しなかった理由は `PipelineTrace.decision_reason` と `decision_source` に残す。

- `decision_source="vad"`: VAD が人声を検出して黙った。
- `decision_source="transcript"`: transcript confidence によって人の発話と判断して黙った。
- `decision_source="event"`: low salience / stale context など event 側の理由で黙った、または event に反応した。

これにより「なぜ黙ったか」「なぜ音に反応したか」を同じ trace で追える。
