...
`review.jsonl` には review 内の出力順 `index` に加えて、元動画側の `frame_index`、`frame_source`、`timestamp`、`frame_path`、`frame_data` を出す。sample interval や開始秒を変えても、どのフレームを見て scene として扱ったかを追えるようにする。

ゲーム固有の理解を行う場合は、`SceneAnalyzer(vision_adapter=...)` に vision model / OCR / object detector を接続する。`run_mp4_commentary()` と `export_mp4_commentary_review()` は `vision_adapter` を直接受け取れる。指定した場合、sampled frame は画像を含んだまま adapter に渡され、adapter の `summary` / `labels` / `ui_elements` / `confidence` が scene analysis に使われる。
