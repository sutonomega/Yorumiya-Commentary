...
MVP では、フレーム解析結果の差分から `CommentaryEvent` まで到達する代表ケースを次で確認する。

| flow | scene diff | expected event | expected metadata / trace |
| --- | --- | --- | --- |
| 戦闘開始 | `field` から `battle` が増える | `combat_state` | `event_phase=combat_start`、`semantic_event=combat_state` |
| 敵出現 | `battle` に `enemy` が増える | `combat_state` | `event_phase=enemy_appeared`、`semantic_event=combat_state` |
| ボス登場 | `battle enemy` に `boss` が増える | `combat_state` | `event_phase=boss_appeared`、`semantic_event=combat_state` |
| 戦闘終了 | combat label が消えて `field` に戻る | `combat_state` | `event_phase=combat_end`、`semantic_event=combat_state` |
| 重要場面 | `critical` / `hit` / `damage` などが増える | `critical_moment` | `semantic_event=critical_moment` |
| 会話開始 | `dialog` / `subtitle` が増える | `dialog_event` | `event_phase=dialog_start`、`semantic_event=dialog_event` |
| 選択肢表示 | `choice` が増える | `dialog_event` | `event_phase=dialog_choice`、`dialog_choice` metadata |
| 会話終了 | dialog label が消える | `dialog_event` | `event_phase=dialog_end` |

これらは固定 comment の網羅ではなく、差分検出から event kind / event_phase / metadata が説明可能に残ることを確認するための受け入れケースである。
`PipelineTrace` は採用後の `event_kind`、`scene_event_phase`、dialog metadata を記録し、`EventSelectionTrace` は scene candidate 段階の `scene_event_kind`、`scene_event_phase`、dialog metadata を記録する。

## Dialog Event Design

`dialog_event` は現時点では単一 kind にまとめる。ただし固定 comment はまだ持たせない。会話や字幕は連続しやすく、「会話が始まったね」のような固定文だけを入れると同じ反応が続きやすいためである。

RPG / ADV 対応を進める段階では、scene metadata に次の情報を入れる。

- `speaker`
- `text`
- `choice`

これらは scene payload の top-level field または `metadata` field で受け取る。`EventDetector` は `dialog_event` の時だけ、event metadata に `dialog_speaker`、`dialog_text`、`dialog_choice` としてコピーする。
...
`dialog_event` の場合だけ、scene metadata から次も入れる。
