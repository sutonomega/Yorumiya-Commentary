...
| `Transcript` | 音声認識 text と time range | `WhisperTranscriber` | `CommentaryContext`, `TranscriptEventDetector` |
| `VadResult` | 人が話しているか | `VoiceActivityDetector` | `CommentGenerator`, scheduler policy |
| `AudioFeatures` | 音量と雰囲気 cue | `AudioAnalyzer` | `EmotionEstimator`, `AudioEventDetector` |
| `CommentaryEvent` | 意味のある変化と salience | `EventDetector`, `AudioEventDetector`, `TranscriptEventDetector` | `EmotionEstimator`, `CommentGenerator` |
...
scene event、audio event、transcript event が同時にある場合、runtime は salience が高い方を `CommentaryContext.event` に採用する。選択結果は `EventSelectionTrace` に残す。

`CommentaryEvent.metadata["source"]` は `scene`、`audio`、`transcript` のいずれかを持つ。これにより downstream は kind の命名に依存せず event の由来を確認できる。
...
- `Transcript.text` と confidence は、VAD が拾えない発話の抑制と transcript event の生成に使う。
- `AudioFeatures.event` と atmosphere は audio event として commentary の判断に使う。
