...
scene event と audio event が同時にある場合、runtime は salience が高い方を `CommentaryContext.event` に採用する。選択結果は `EventSelectionTrace` に残す。

`CommentaryEvent.metadata["source"]` は `scene` または `audio` を持つ。これにより downstream は kind の命名に依存せず event の由来を確認できる。
...
- `Transcript.text` と confidence は、VAD が拾えない発話の抑制に使う。
